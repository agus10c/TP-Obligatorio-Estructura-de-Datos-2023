...
public class MapeoAMuchos {
    ...
    public boolean asociar(Object clave, Object valor) {
        ...
        int pos = Math.abs(clave.hashCode()) % TAM; //Obtengo la posicion hash de la clave

        NodoHashMapeoM aux = tabla[pos];

        while (aux != null && !aux.getDominio().equals(clave)) { //busco el nodo que contenga la clave
            aux = aux.getEnlace();
        }
    ...
    public boolean desasociar(Object clave, Object valor) {
        ...
        while (aux != null && !aux.getDominio().equals(clave)) { //Obtengo la posicion hash de la clave
            aux = aux.getEnlace();
        }
    ...
    private NodoHashMapeoM obtenerNodoConDominio(Object clave) {
        //retorna el nodo que contiene el dominio con el objeto clave ingresado
        int pos = Math.abs(clave.hashCode()) % TAM; //Obtengo la posicion hash de la clave
    ...
    public SolicitudDeViaje obtenerSolicitud(int co, int cd, int num) {
        //Retorna la solicitud con numero de solicitud num, si no la encuentra retorna null
        Lista solicitudes = obtenerValor(co + "-" + cd);
        SolicitudDeViaje solicitud = null;

        boolean encontrado = false;

        int pos = buscarSolicitud(solicitudes, num);
    ...
    private int buscarSolicitud(Lista lista, int num) {
        // busca la solicitud con numero de solicitud num en la lista. Si la encuentra la retorna y sino retorna null
        int pos = 1;
        boolean encontrado = false;
    ...
    public Lista listarPosiblesSolicitudesIntermedias(Lista camino, int espacioDisponible) {
        ...
        for (int i = 1; i <= camino.longitud() - 1; i++) {
            ...
            for (int j = i + 1; j <= camino.longitud(); j++) {
                ...
                for (int k = 1; k <= solicitudes.longitud(); k++) {
    ...
    public Lista verificarCaminoPerfecto(Lista camino, int espacioDisponible) {
        //retorna la lista de solicitudes que hace que el camino sea perfecto. De no ser un camino perfecto retorna una lista vacia
        Lista lisSolicitudes = new Lista();
        if (!verificarCaminoPerfectoAux(lisSolicitudes, espacioDisponible, camino, 1)) {
            lisSolicitudes.vaciar();
        }
        return lisSolicitudes;
    }

    private boolean verificarCaminoPerfectoAux(Lista lisSolicitudes, int espacioDisponible, Lista camino, int pos) {
        ...
        if (pos >= camino.longitud()) {
            ...
            int origen = (int) camino.recuperar(pos);
            
            if(!lisSolicitudes.esVacia()) {
                //Busco si hay alguna solicitud que tenga destino a la ciudad actual para desocupar el espacio
                boolean encontrado = false;
                int i = 1;
                SolicitudDeViaje s;
                
                while(!encontrado && i <= lisSolicitudes.longitud()){
                    
                    s = (SolicitudDeViaje) lisSolicitudes.recuperar(i);
                    
                    if(s.getCiudadDestino() == origen) {
                        encontrado = true;
                        espacioDisponible = espacioDisponible + s.getMetrosCubicos(); //Desocupo el espacio
                    }
            ...
            int i = pos + 1;
            while (!exito && i <= camino.longitud()) {

                int destino = (int) camino.recuperar(i);
                Lista solicitudes = this.obtenerValor(origen + "-" + destino);

                int j = 1;
                while (!exito && j <= solicitudes.longitud()) {

                    SolicitudDeViaje s = (SolicitudDeViaje) solicitudes.recuperar(j);
                    int espacioDeSolicitud = s.getMetrosCubicos();

                    if (espacioDeSolicitud <= espacioDisponible) {
                        
                        lisSolicitudes.insertar(s, pos);
                        exito = verificarCaminoPerfectoAux(lisSolicitudes, espacioDisponible - espacioDeSolicitud, camino, pos + 1);
                        
                        if (!exito) {
                            lisSolicitudes.eliminar(pos);
                        }
